...
const Home = () => {
  ...
  const handleBookRide = async () => {
    ...
    try {
      // Load drivers from localStorage 'verified_drivers'
      const drivers: Driver[] = loadVerifiedDrivers();

      // Filter for verified drivers only
      const verifiedDrivers: Driver[] = drivers.filter(
        (driver: Driver) => driver.verified === true,
      );

      if (verifiedDrivers.length === 0) {
        // No verified drivers available
        setError(
          'No verified drivers available right now. Please try again later.',
        );
        setIsLoading(false);
        return;
      }
